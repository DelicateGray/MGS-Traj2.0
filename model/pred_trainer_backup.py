...
class PredTrainer:
    ...
    def __init__(self, config):
        ...
        test_set = joblib.load(f"{self.data_dir}/test_loader.pkl")
    ...
    def train_step(self):
        ...
        for i, (traj, traj_mask) in enumerate(self.train_data):
            ...
            if i < int(train_batch_num * self.sample_ratio):
                traj_his = traj[:, :, : self.config['common']['Th'], :].to(self.device, non_blocking=True)
                traj_his_mask = traj_mask[:, :, : self.config['common']['Th']].to(self.device, non_blocking=True)
                target = traj[:, 0, -self.config['common']['Tf']:, :2].to(self.device, non_blocking=True)
                
                pred = self.model(traj_his, traj_his_mask)
                loss = self.criterion.compute_loss(pred, target)
                
                TrainLosses["Total"].append(loss.item()) # 存储 item 以减少内存占用
                ...
                if (i + 1) % print_interval == 0:
                    ...
                    train_bar.set_description(
                        f"<Training>  [Learning Rate]: {lr:.2e}  [Train Loss]: {avg_loss:.6f}"
                    )
                    running_loss = 0.0
            else:
                break
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            ...
            for i, ((traj, traj_mask), meta) in enumerate(zip(self.val_data, self.val_meta)):
                train_bar_update = True # 标记是否需要更新 bar，因为 zip 可能会导致长度不一致，这里简单处理
                val_bar.update(1)
                
                if i < int(val_batch_num * self.sample_ratio):
                    traj_his = traj[:, :, : self.config['common']['Th'], :].to(self.device, non_blocking=True)
                    traj_his_mask = traj_mask[:, :, : self.config['common']['Th']].to(self.device, non_blocking=True)
                    target = traj[:, 0, self.config['common']['Th']:, :2].to(self.device, non_blocking=True)
                    
                    pred = self.model(traj_his, traj_his_mask)
                    loss = self.criterion.compute_loss(pred, target)
                    ValLosses["Total"].append(loss.item())
                    ...
                    if (i + 1) % print_interval == 0:
                        ...
                        val_bar.set_description(
                            f"<Validation>  [Val Loss]: {avg_loss:.6f}"
                        )
                        running_loss = 0.0
                else:
                    break
    ...
    def test(self, model_path=None):
        ...
        if model_path is None:
            model_path = self.config["pred"]["test_model_path"]
