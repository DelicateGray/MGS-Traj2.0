...
class VisualizationPlot(object):
    def __init__(self, arguments, read_tracks, static_info, meta_dictionary, fig=None):
        ...
        if background_image_path is not None and os.path.exists(background_image_path):
            # Plot the image
            self.background_image = imageio.imread(background_image_path)
            self.y_sign = 1
        ...
        # Define the widgets
        self.frame_slider = DiscreteSlider(
            self.ax_slider,
            "Frame",
            1,
            self.maximum_frames,
    ...
    def update_button_next(self, _):
        if self.current_frame < self.maximum_frames:
            ...
            print(
                "There are no frames available with an index higher than {}.".format(
                    self.maximum_frames
                )
            )  #
        ...
        for track in self.tracks:
            ...
            if initial_frame <= self.current_frame < final_frame:
                ...
                if self.arguments["plotDirectionTriangle"]:
                    ...
                    polygon = plt.Polygon(
                        xy=np.transpose(triangle_info), closed=True, **triangle_style
                    )
    ...
    def on_click(self, event):
        ...
        try:
            ...
            print(
                "Something went wrong when trying to plot the detailed information of the vehicle."
            )
            return
    ...
    # /.............................保存为视频.................................../
    def save_animation(self, filename, fps=30, frame_range=None):
        print(f"Max frame range: [0,{self.maximum_frames}]")
        self.frame_range = (
            frame_range
            if frame_range is not None
            else range(1, self.maximum_frames + 1)
        )
        frames = []
        for frame_number in self.frame_range:
